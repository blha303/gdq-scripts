...
function objectToHTML(json) {
	var i, key, htmlKey, length, keys = Object.keys(json), output = '<div class="collapser"></div>{<span class="ellipsis"></span><ul class="obj collapsible">', hasContents = false;
	for (i = 0, length = keys.length; i < length; i++) {
		key = keys[i];
		hasContents = true;
		output += '<li><div class="hoverable">';
                htmlKey = htmlEncode(key);
		output += '<a name="' + htmlKey + '"><span class="property">' + htmlKey + '</span></a>: ';
